      ...
      >
        ...
        <el-table-column label="商品图片" width="160" align="center">
          <template slot-scope="scope"
            ><img class="productListImg" style="height: 80px" :src="scope.row.selectProductPics[0].url"
          /></template>
        </el-table-column>
  ...
  methods: {
    ...
    handleDelete(index, row) {
      this.$confirm("是否要进行删除操作?", "提示", {
        ...
        let ids = [];
        ids.push(row._id);
        deleteProduct({ids: ids}).then(res=>{
          this.$message({
            message: res.message,
            type: "success"
          });
    ...
    handleUpdateProduct(index, row) {
      this.$router.push({ path: "/pms/updateProduct", query: { id: row._id } });
    },
